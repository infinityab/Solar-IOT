...
If contactor switching is required at the power board (for large HWS's or heavily inductive loads) the associated Pi ports are buffered and drive optically isolated 4 x relay module boards thus providing 4 x 240V/110V AC switchable outlets which in turn directly trigger the contactors on or off. The contactors are simply direct replacements for your trip switches and are exactly the same size, it is recommended that you use contactors that also have a manual overide. If only clientside WiFi or Wireless switching is to be used then the buffer and relay board is not required.

Remote appliances are driven by WiFi SONOFF wall adapter sockets (S20) or SONOFF switch boxes or WiFi relay modules. Be aware that ratings quoted on relays are for resistive loads, inductive loads will be about a quarter of that. See the hardware folder. Commands may be for ON or OFF or Pulse. Pulse is for appliances that have 'push start' buttons (aircons etc).

Wireless remote Awnings or other wireless appliances may also be used by simply adding a 433Mhz transmitter module inside a WiFi SONOFF switch box and connected to the green LED port. The wireless trigger codes may discovered using Arduino desktop IDE software with 'ESP8266 RC-Switch' software and 433MHZ receiver attached or the Audacity audio PC app connecting the RX output directly to audio input (you may have to attenuate it) to discover packet length and bit orientation (BTW they use a NRZ Mmanchester coding). The RX receiver and transmitter are very cheap devices costing about $1.
...
So for instance water heater 2400 watts #1 priority, Fan Heater 1000W (by WIfi socket) or AirCon 1800W (by WiFi module) so these could be set to #2 priority, pool pump 1200W this set to say #3 priority. Priority, the Auto # dictates the order of power calculation. Thus in this example once the water heater power is used up and there is still surplus solar the calculation can be carried forward and if say 1Kw surplus is still available a signal is transmitted to the wireless socket for the heater and it switches on. This is recalculated at 3 minute interval (settable) so if the power drops it will be commanded to switch off. 
